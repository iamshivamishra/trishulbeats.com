...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const staticPages: MetadataRoute.Sitemap = [
    ...
    { url: `${baseUrl}/beat-packs`, lastModified: new Date(), changeFrequency: "daily", priority: 0.8 },
  ...
  const [beats, producers, beatPacks] = await Promise.all([
    Beat.find({
      isPublished: true,
    ...
    BeatPack.find({ status: "published", isPublished: true })
      .select("_id updatedAt")
      .sort({ updatedAt: -1 })
      .limit(500)
      .lean<{ _id: string; updatedAt: Date }[]>(),
  ...
  const beatPackPages: MetadataRoute.Sitemap = beatPacks.map((pack) => ({
    url: `${baseUrl}/beat-packs/${pack._id}`,
    lastModified: pack.updatedAt,
    changeFrequency: "weekly" as const,
    priority: 0.7,
  }));

  return [...staticPages, ...beatPages, ...producerPages, ...beatPackPages];
}
